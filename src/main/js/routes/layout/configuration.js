...
function Configuration({ loadedVespaState }) {
  ...
  const [routingConfigurationState, setRoutingConfigurationState] =
    useState(initialState);

  return (
    <dialog id="vispana-config-modal" className="modal">
      <div
      ...
      >
        ...
        <div className="w-full">
          ...
          >
            {Array.from(routingConfigurationState.entries()).map((entry) => {
              const entryKey = entry[0];
              const entryValue = entry[1];
              return (
                <div className="mb-1" key={entryKey}>
                  <div className="label">
                    <label
                      htmlFor="cluster"
                  ...
                  <input
                    ...
                    placeholder={`e.g., http://${entryKey}-cluster-lb/`}
                  ></input>
                </div>
              );
            })}
