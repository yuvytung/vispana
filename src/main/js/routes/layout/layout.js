...
function Layout() {
  ...
  return (
    <>
      <Suspense fallback={<Loading />}>
        <Await resolve={loaderData.vespaState} errorElement={<VispanaError />}>
          {(loadedVespaState) => (
            <main role="main" className="h-screen flex flex-row flex-wrap">
              <div
                id="sideBar"
                className="relative h-screen  flex flex-col flex-wrap p-6 min-w-75 max-w-80 animated faster bg-standout-blue"
              >
                <div className="flex flex-col">
                  <div className="flex flex-row flex-start justify-center mb-3 font-medium text-sm hover:text-teal-600 transition ease-in-out duration-500">
                  ...
                  <NavLink
                    to={`config?${searchParams.toString()}`}
                  ...
                  <NavLink
                    to={`container?${searchParams.toString()}`}
                    className={navLinkStyle()}
                  >
                  ...
                  <NavLink
                    to={`content?${searchParams.toString()}`}
                    className={navLinkStyle()}
                  >
                  ...
                  <NavLink
                    to={`apppackage?${searchParams.toString()}`}
                    className={navLinkStyle()}
                  >
                  ...
                  <Accordion
                    controllerElement={accordionHeaderController}
                    contentDescription="schemas"
                  >
                    {loadedVespaState.content.clusters.flatMap((cluster) => {
                      return cluster.contentData.map((contentData) => {
              ...
              <div className="flex-1 h-screen p-6 overflow-x-auto w-full">
                <div>
                  <div className="space-x-1 w-full parent-table">
                    <div className="text-left font-flow left-child">
                      {hostName()}
                    </div>
                    <div
                      className="text-right font-flow right-child"
                      style={{ textAlign: "right" }}
                    >
                      <a
                        className="text-xs btn btn-square btn-sm bg-standout-blue border-0 hover:bg-standout-blue hover:border-0 active:border-0"
                        style={{ marginRight: "2px" }}
                        onClick={() => {
                          document
                            .getElementById("vispana-config-modal")
                            .showModal();
                        }}
                      >
                      ...
                      <select
                        ...
                        id="form_interval"
                        name="refresh_interval[interval]"
                        defaultValue={-1}
                        onChange={(event) => {
              ...
              <Configuration
                loadedVespaState={loadedVespaState}
              ></Configuration>
            </main>
          )}
        </Await>
  ...
  function navLinkStyle() {
    return ({ isActive }) => {
      if (isActive) {
        return (
          "mt-3 font-medium text-sm transition ease-in-out duration-100" +
          " text-yellow-400 text-ellipsis overflow-hidden w-full"
        );
      } else {
        return (
          "mt-3 font-medium text-sm hover:text-white transition" +
...
export async function loader({ request }) {
  ...
  const vespaState = getVespaState(configHost).then((vespaState) => {
    vespaState.container.clusters.forEach((cluster) => {
      ...
      if (url.searchParams.has(routingSearchParamName(containerName))) {
        cluster.route = url.searchParams.get(
          routingSearchParamName(containerName),
        );
      } else {
