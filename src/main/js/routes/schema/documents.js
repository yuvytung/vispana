...
function Documents({containerUrl, schema, editDocument, onEditConsumed}) {
    ...
    useEffect(() => {
        ...
        const docIdField = editDocument.documentid || editDocument.id || editDocument.document_id;
        if (docIdField) {
            ...
            if (parsed) {
                ...
                fetchAndFillDocument(parsed.namespace, parsed.documentType, parsed.documentId);
            } else {
    ...
    async function fetchAndFillDocument(ns, docType, docId) {
        ...
        setLoading(true);
        try {
            const response = await vispanaClient.getDocument(containerUrl, ns, docType, docId);
            if (response.fields) {
            ...
            setError({hasError: true, error: err.message || "Failed to fetch document for editing"});
        } finally {
            setLoading(false);
        }
    }
    ...
    function parseVespaDocumentId(input) {
        ...
        if (match) {
            return {namespace: match[1], documentType: match[2], documentId: match[3]};
        }
        return null;
    }
    ...
    async function handleOperation(operationName, operationFn) {
        ...
        try {
            ...
            if (response.error || response.message?.toLowerCase().includes("error")) {
                setError({hasError: true, error: response.message || JSON.stringify(response)});
            } else {
                setSuccess(`${operationName} completed successfully.`);
            }
            setResult(response);
        } catch (err) {
            setError({hasError: true, error: err.message || "An unexpected error occurred"});
        } finally {
            setLoading(false);
        }
    }
    ...
    function handleGet() {
        handleOperation("Get", () =>
            vispanaClient.getDocument(containerUrl, namespace, documentType, documentId)
        );
    }

    function handleSave() {
        handleOperation("Save", () =>
            vispanaClient.saveDocument(containerUrl, namespace, documentType, documentId, documentBody)
        );
    }
    ...
    function handleDelete() {
        ...
        handleOperation("Delete", () =>
            vispanaClient.deleteDocument(containerUrl, namespace, documentType, documentId)
        );
    }
        ...
        <div className="p-4 min-w-full">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                <div className="form-control">
                    ...
                    <input id="doc-namespace" type="text" placeholder="namespace"
                ...
                <div className="form-control">
                    ...
                    <input id="doc-type" type="text" placeholder="document type"
                           className="input input-bordered input-sm bg-standout-blue text-white w-full"
                ...
                <div className="form-control">
                    ...
                    <input id="doc-id" type="text" placeholder="document ID or id:namespace:type::id"
                           className="input input-bordered input-sm bg-standout-blue text-white w-full"
                           value={documentId} onChange={e => handleDocumentIdChange(e.target.value)}/>
                </div>
            </div>

            <div className="form-control mb-3">
                <label className="label" htmlFor="doc-body">
                    <span className="label-text text-yellow-400">Document Body (JSON)</span>
                </label>
                <textarea id="doc-body" placeholder='{"fields": { ... }}'
                          className="textarea textarea-bordered bg-standout-blue text-white w-full font-mono text-sm"
                          rows={8}
                          value={documentBody} onChange={e => setDocumentBody(e.target.value)}/>
            </div>

            <div className="flex flex-row gap-2 mb-4 justify-end">
                <button className="btn btn-sm bg-standout-blue text-yellow-400 border-none outline-none"
                        disabled={loading} onClick={handleGet}>
                ...
                <button className="btn btn-sm bg-red-700 text-white border-none outline-none"
                        disabled={loading} onClick={handleDelete}>
                    <i className="fas fa-trash pr-1"/> Delete
                </button>
            </div>

            {loading && <div className="text-center text-yellow-400 py-4">Loading...</div>}
