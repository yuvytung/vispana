...
function VispanaError({ errorMessage, showLogo = true }) {
  ...
  errorMessage = errorMessage
    ...
    : {
        ...
        description:
          "Make sure you are pointing correctly to a Vespa cluster with a running application.",
      };
  ...
  return (
    <>
      <main role="main" className="h-screen flex flex-row flex-wrap min-w-full">
        <div
          className={`${alignCenter} min-h-screen bg-darkest-blue min-w-full`}
        >
          <div className="flex-col justify-center hero-content min-w-full">
            <div className="card flex-shrink-0 w-full max-w-1/2 shadow-2xl bg-standout-blue overflow-visible">
              <div
                style={{
                  position: "absolute",
                  top: "-50px",
                ...
                {showLogo ? (
                  <div className="mb-3 w-24 h-24 rounded-full bg-white flex items-center justify-center cursor-pointer text-indigo-700 border-4 border-yellow-400 overflow-hidden">
                    <a href="/">
                      <img
                        alt=""
                        src="/img/vispana-broken-logo.png"
              ...
              <div
                className={`flex ${showLogo ? "mt-10" : ""} card-body w-800 text-center`}
              >
                <span className="text-yellow-400">Opzzz..</span>
                {error && error.status === 404 ? (
                  <div>Page not found x_x</div>
                ) : (
                  genericError
                )}
              </div>
