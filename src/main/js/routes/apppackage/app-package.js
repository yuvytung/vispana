...
function AppPackage() {
  ...
  const schemas = vespaState.content.clusters
    .flatMap((cluster) => {
      return cluster.contentData.map((data) => {
        return {
    ...
    content: (
      <div className="mt-8 mb-3">
        <p>
          ...
          {vespaState.applicationPackage.appPackageGeneration}
        </p>
        <p>
          <span className="text-yellow-400">Version:</span>{" "}
          {`${vespaState.vespaVersion.major}.${vespaState.vespaVersion.minor}.${vespaState.vespaVersion.patch}`}
        </p>
      </div>
    ),
  });
  ...
  return (
    <div className="flex-1 max-h-full max-w-full bg-darkest-blue">
      <div
        className="flex flex-grow flex-col pt-2 normal-case"
        style={{ minWidth: "0" }}
      >
