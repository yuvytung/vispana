...
function DynamicEnhancedGrid({
        ...
        cols.push({
          ...
          cell: (row) => (
            <div className="flex items-center">
              ...
              {onEditDocument && (
                <EditButton data={row} onEdit={onEditDocument} />
              )}
            </div>
          ),
  ...
  const safeOnChangeRowsPerPage = useMemo(() => {
    return typeof onChangeRowsPerPage === "function"
      ? onChangeRowsPerPage
      : () => {
          console.warn(
            "onChangeRowsPerPage handler not provided or not a function",
          );
        };
  }, [onChangeRowsPerPage]);

  const safeOnChangePage = useMemo(() => {
    return typeof onChangePage === "function"
      ? onChangePage
      : () => {
  ...
  const safePaginationRowsPerPageOptions = useMemo(() => {
    if (!Array.isArray(paginationRowsPerPageOptions)) {
      console.warn(
        "paginationRowsPerPageOptions is not an array, using default",
      );
  ...
  const safePaginationPerPage = useMemo(() => {
    const perPage =
      typeof paginationPerPage === "number" && paginationPerPage > 0
        ? paginationPerPage
        : 15;
    ...
    if (!safePaginationRowsPerPageOptions.includes(perPage)) {
      console.warn(
        `paginationPerPage (${perPage}) not in options, using first option`,
      );
