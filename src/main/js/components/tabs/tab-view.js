...
function TabView({ tabs, currentTab, tabSelector }) {
  ...
  tabs.forEach((tab) => {
    if (!tab.hasOwnProperty("header") || !tab.hasOwnProperty("content")) {
      throw new Error(
        `Expected 'header' and 'content' properties in TabView. Got: ${tab}`,
      );
    }
  });
    ...
    <div className="w-full max-w-full min-w-full">
      <Tabs selectedIndex={currentTab} onSelect={(index) => tabSelector(index)}>
        <TabList className="flex justify-left items-left component-no-scrollbar overflow-x-scroll">
          {tabs.map((tab, index) => (
            <Tab
              className="min-w-fit cursor-pointer py-2 px-4 border-b-2 text-gray-500 border-gray-500 "
          ...
          <Tab
            key={tabs.length + 1}
            className="flex-grow text-xs py-2 px-4 text-gray-500 border-gray-500 border-b-2"
            disabled={true}
          ></Tab>
        </TabList>
