...
public class VespaStateClient {
  ...
  public VispanaRoot vespaState(String configHost) {
    try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
      ...
      var vespaVersionFork = scope.fork(() ->
        VespaVersionFetcher.fetch(configHost)
      );
      var vespaMetricsFork = scope.fork(() ->
        MetricsFetcher.fetchMetrics(configHost)
      );
      var appUrlFork = scope.fork(() ->
        ApplicationUrlFetcher.fetch(configHost)
      );
      scope
        .join()
        .throwIfFailed(throwable ->
          new RuntimeException(
            "Failed to get prerequisites from Vespa",
            throwable
          )
        );
      ...
      var appPackageScope = scope.fork(() ->
        AppPackageAssembler.assemble(appUrl)
      );
      var configFork = scope.fork(() ->
        ConfigNodesAssembler.assemble(configHost, vespaMetrics)
      );
      scope
        .join()
        .throwIfFailed(throwable ->
          new RuntimeException("Failed to get data from Vespa", throwable)
        );
      ...
      var containerFork = scope.fork(() ->
        ContainerAssembler.assemble(configHost, vespaMetrics)
      );
      var contentFork = scope.fork(() ->
